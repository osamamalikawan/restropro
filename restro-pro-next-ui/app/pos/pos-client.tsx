...
export function PosClient({
  ...
  return (
    <main className="pos-shell min-h-screen text-neutral-50 flex flex-col md:flex-row">
      <div className="flex-1 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="font-display text-xl font-semibold">{restaurantName} — POS</h1>
            <p className="text-neutral-500 text-xs">Cashier: {cashierName}</p>
          </div>
        ...
        {products === null ? (
          <p className="text-neutral-500 text-sm">Loading menu…</p>
        ) : products.length === 0 ? (
            ...
            {products
              ...
              .map((p) => (
                <button
                  key={p.id}
                  onClick={() => addToCart(p)}
                  className="pos-product rounded-xl border p-4 text-left transition-all"
                >
      ...
      <div className="pos-order w-full md:w-80 border-t md:border-t-0 md:border-l p-6 flex flex-col">
          ...
          {ORDER_TYPES.map((t) => (
            <button
              ...
              className={`rounded-lg py-2 text-xs font-semibold border ${
                orderType === t.id ? "bg-chili-500/20 border-chili-500 text-chili-400" : "border-neutral-700 text-neutral-400"
              }`}
            >
              {t.label}
            </button>
        ...
        {orderType === "dine_in" && (
          <select value={tableId} onChange={(e) => setTableId(e.target.value)} className="w-full rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm mb-3">
        ...
        {orderType === "delivery" && (
          <select value={areaId} onChange={(e) => onAreaChange(e.target.value)} className="w-full rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm mb-3">
        ...
        <div className="flex-1 overflow-y-auto space-y-2 mb-4">
          {cart.length === 0 ? (
            <p className="text-neutral-500 text-sm">Tap a product to start an order.</p>
          ) : (
            cart.map((l) => (
              <div key={l.productId} className="flex items-center justify-between text-sm border-b border-neutral-800 pb-2">
                <span>{l.name}</span>
                <div className="flex items-center gap-2">
                  <button onClick={() => changeQty(l.productId, -1)} className="w-5 h-5 rounded bg-neutral-800 hover:bg-neutral-700">
                    −
                  </button>
        ...
        <div className="border-t border-neutral-800 pt-3 space-y-1 text-sm mb-4">
          ...
          <div className="flex justify-between font-semibold text-base pt-1 border-t border-dashed border-neutral-700">
            <span>Total</span>
            <span>Rs {total}</span>
          </div>
        </div>
      ...
      {checkoutOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-sm rounded-xl border border-neutral-800 bg-neutral-900 p-6 space-y-4">
            ...
            <div>
              <label className="text-xs uppercase tracking-wide text-neutral-500">Customer</label>
              {selectedCustomer ? (
                <div className="flex items-center justify-between rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 mt-1 text-sm">
                  <span>
                    {selectedCustomer.name} <span className="text-neutral-500">· {selectedCustomer.phone}</span>
                  </span>
                  <button onClick={() => setSelectedCustomer(null)} className="text-neutral-500 hover:text-crimson-400">
                    ✕
                  </button>
                </div>
              ) : (
                <>
                  <input
                    ...
                    className="w-full rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm mt-1"
                  />
                  {custResults.length > 0 && (
                    <div className="mt-1 max-h-32 overflow-y-auto rounded-md border border-neutral-700">
                      {custResults.map((c) => (
                        <button
                          key={c.id}
                          onClick={() => selectCustomer(c)}
                          className="w-full text-left px-3 py-2 text-sm hover:bg-neutral-800 border-b border-neutral-800 last:border-b-0"
                        >
                          {c.name} <span className="text-neutral-500">· {c.phone}</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {showNewCustFields && (
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <input
                        ...
                        placeholder="Name"
                        className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm"
                      />
                      <input
                        value={newCustPhone}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 rounded-lg bg-neutral-800 hover:bg-neutral-700 py-2.5 font-semibold"
              >
                Cancel
              </button>
              <button
