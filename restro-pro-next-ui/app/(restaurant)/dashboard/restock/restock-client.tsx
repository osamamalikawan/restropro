    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-50 p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5 mb-6">
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <select value={itemId} onChange={(e) => setItemId(e.target.value)} className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <input value={quantity} onChange={(e) => setQuantity(e.target.value)} type="number" placeholder="Quantity received" className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm" />
          <input value={unitCost} onChange={(e) => setUnitCost(e.target.value)} type="number" placeholder="Cost per unit" className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm" />
        </div>
          ...
          <tbody>
            {purchases.map((p) => (
              <tr key={p.id} className="border-t border-neutral-800">
                <td className="p-3 text-neutral-400">{p.purchased_at}</td>
                <td className="p-3 font-medium">{p.inventory_items?.name}</td>
                <td className="p-3 text-neutral-400">{p.suppliers?.name ?? "—"}</td>
                <td className="p-3">
