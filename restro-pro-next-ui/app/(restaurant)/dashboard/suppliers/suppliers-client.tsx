    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-50 p-6 md:p-8 max-w-3xl mx-auto">
            ...
            {suppliers.map((s) => (
              <tr key={s.id} className="border-t border-neutral-800">
                ...
                <td className="p-3 text-neutral-400">
                  {s.contact_person}
                  {s.phone ? <div className="text-xs text-neutral-500">{s.phone}</div> : null}
                </td>
                <td className="p-3 text-neutral-400">{s.category}</td>
                <td className="p-3 text-neutral-400">{s.payment_terms}</td>
                <td className="p-3">
                  <button onClick={() => removeSupplier(s.id)} className="text-neutral-500 hover:text-crimson-400 text-xs">
                    Remove
                  </button>
                </td>
              </tr>
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5">
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Company name" className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm" />
          <input value={contact} onChange={(e) => setContact(e.target.value)} placeholder="Contact person" className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm" />
        </div>
        <div className="grid grid-cols-3 gap-2 mb-3">
          <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone" className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm" />
          <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Category" className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm" />
          <select value={terms} onChange={(e) => setTerms(e.target.value)} className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-sm">
            <option>Net 7</option>
