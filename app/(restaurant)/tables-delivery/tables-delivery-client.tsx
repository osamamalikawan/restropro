      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="rounded-xl border border-line bg-surface p-5">
          ...
          <div className="space-y-2 mb-4 max-h-64 overflow-y-auto">
            {tables.map((t) => (
              <div key={t.id} className="flex items-center justify-between text-sm border-b border-line pb-2">
                <span>
            ...
            {tables.length === 0 && <p className="text-ink-faint text-sm">No tables yet.</p>}
          </div>
          <div className="flex gap-2">
            <input value={tableNo} onChange={(e) => setTableNo(e.target.value)} placeholder="e.g. 12" className="flex-1 rounded-md bg-raised border border-line px-3 py-2 text-sm" />
            <input value={tableSeats} onChange={(e) => setTableSeats(e.target.value)} type="number" placeholder="Seats" className="w-20 rounded-md bg-raised border border-line px-3 py-2 text-sm" />
        ...
        <div className="rounded-xl border border-line bg-surface p-5">
          ...
          <div className="space-y-2 mb-4 max-h-64 overflow-y-auto">
            {areas.map((a) => (
              <div key={a.id} className="flex items-center justify-between text-sm border-b border-line pb-2">
                <span>
            ...
            {areas.length === 0 && <p className="text-ink-faint text-sm">No areas yet.</p>}
          </div>
          <div className="flex gap-2">
            <input value={areaName} onChange={(e) => setAreaName(e.target.value)} placeholder="e.g. Gulgasht Colony" className="flex-1 rounded-md bg-raised border border-line px-3 py-2 text-sm" />
            <input value={areaFee} onChange={(e) => setAreaFee(e.target.value)} type="number" placeholder="Fee" className="w-20 rounded-md bg-raised border border-line px-3 py-2 text-sm" />
