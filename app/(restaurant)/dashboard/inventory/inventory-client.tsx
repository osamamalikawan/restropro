    ...
    <main className="min-h-screen bg-canvas text-ink-strong p-6 md:p-8 max-w-3xl mx-auto">
            ...
            {items.map((i) => {
              ...
              return (
                <tr key={i.id} className="border-t border-line">
                  ...
                  <td className="p-3 text-ink-faint">{i.min_stock}</td>
                  <td className="p-3 text-ink-mid">Rs {i.cost}</td>
                  <td className="p-3">
                    <button onClick={() => removeItem(i.id)} className="text-ink-faint hover:text-crimson-400 text-xs">
                      Remove
                    </button>
                  </td>
                </tr>
      ...
      <div className="rounded-xl border border-line bg-surface p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name" className="rounded-md bg-raised border border-line px-3 py-2 text-sm md:col-span-2" />
          <select value={unit} onChange={(e) => setUnit(e.target.value)} className="rounded-md bg-raised border border-line px-3 py-2 text-sm">
            <option>pcs</option>
            <option>kg</option>
            <option>litre</option>
            <option>box</option>
          </select>
          <input value={stock} onChange={(e) => setStock(e.target.value)} placeholder="Stock" type="number" className="rounded-md bg-raised border border-line px-3 py-2 text-sm" />
          <input value={minStock} onChange={(e) => setMinStock(e.target.value)} placeholder="Min stock" type="number" className="rounded-md bg-raised border border-line px-3 py-2 text-sm" />
          <input value={cost} onChange={(e) => setCost(e.target.value)} placeholder="Cost/unit" type="number" className="rounded-md bg-raised border border-line px-3 py-2 text-sm" />
        </div>
