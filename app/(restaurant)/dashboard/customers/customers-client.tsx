  ...
  return (
    <main className="min-h-screen bg-canvas text-ink-strong p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <input
        ...
        className="w-full rounded-md bg-raised border border-line px-3 py-2 text-sm mb-4"
      />
          ...
          <tbody>
            {customers.map((c) => (
              <tr key={c.id} className="border-t border-line">
                ...
                <td className="p-3 text-ink-mid">{c.phone}</td>
                <td className="p-3 text-ink-mid">{c.address ?? "—"}</td>
                <td className="p-3 text-ink-mid">{c.delivery_areas?.name ?? "—"}</td>
                <td className="p-3">
                  <button onClick={() => removeCustomer(c.id)} className="text-ink-faint hover:text-crimson-400 text-xs">
                    Remove
                  </button>
                </td>
              </tr>
      ...
      <div className="rounded-xl border border-line bg-surface p-5">
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          ...
          <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone (unique)" className="rounded-md bg-raised border border-line px-3 py-2 text-sm" />
        </div>
        <div className="grid grid-cols-2 gap-2 mb-3">
          <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address" className="rounded-md bg-raised border border-line px-3 py-2 text-sm" />
          <select value={areaId} onChange={(e) => setAreaId(e.target.value)} className="rounded-md bg-raised border border-line px-3 py-2 text-sm">
