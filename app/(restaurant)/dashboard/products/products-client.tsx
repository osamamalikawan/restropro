    ...
    <main className="min-h-screen bg-canvas text-ink-strong p-6 md:p-8">
      ...
      >
        ...
        <Field label="Deal price (Rs)">
          <input value={dPrice} onChange={(e) => setDPrice(e.target.value)} type="number" min="0" step="0.01" className={`${inputCls} max-w-[160px]`} placeholder="0.00" />
        </Field>
        ...
        <div>
          ...
          <div className="space-y-2">
            {dComponents.map((c, idx) => {
              const usedElsewhere = new Set(dComponents.filter((_, i) => i !== idx).map((r) => r.productId));
              const rowOptions = componentOptions.filter((p) => p.id === c.productId || !usedElsewhere.has(p.id));
              return (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    title="Remove"
                    className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-ink-faint transition hover:bg-raised hover:text-crimson-400"
                  >
                    ✕
                  </button>
                </div>
          ...
          <button
            ...
            disabled={dComponents.length >= componentOptions.length}
            className="mt-2 text-xs font-medium text-chili-500 hover:underline disabled:opacity-50 disabled:no-underline"
          >
            + Add product to deal
          </button>
          {componentOptions.length === 0 && <p className="mt-1 text-[11px] text-ink-faint">Add some regular products first, then come back to build a deal.</p>}
        </div>
